...
def upload_model_to_huggingface(checkpoint_path: str = "checkpoints/fine_tuned_prithvi_multimodal.pth", repo_id: str = settings.HF_MODEL_ID):
    ...
    if not token:
        print("⚠️ HF_TOKEN not set in environment or settings. Skipping live Hugging Face upload.")
        return False
        
    api = HfApi()
    print(f"🤗 Uploading {checkpoint_path} to Hugging Face Hub repo: {repo_id}...")
    api.upload_file(
